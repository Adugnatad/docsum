...
const DEFAULT_FOCUS_POINTS: FocusPoint[] = [
  ...
  { id: 'overview', label: 'Overview', isSelected: false },
  { id: 'tech-highlights', label: 'Technical Highlights', isSelected: false },
];
...
export const DocSumProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [focusPoints, setFocusPoints] = useState<FocusPoint[]>(DEFAULT_FOCUS_POINTS);
  ...
  const [processingStep, setProcessingStep] = useState<string>('Parsing content...');
  const [isHelpOpen, setIsHelpOpen] = useState(false);
  const [isExportOpen, setIsExportOpen] = useState(false);

  useEffect(() => {
    try {
      const saved = localStorage.getItem('docsum_history');
      if (saved) {
  ...
  const saveToHistory = (newSummary: SummaryResult) => {
    const updated = [newSummary, ...history.filter((h) => h.id !== newSummary.id)].slice(0, 20);
    setHistory(updated);
    try {
      localStorage.setItem('docsum_history', JSON.stringify(updated));
    } catch (e) {
  ...
  const clearHistory = () => {
    setHistory([]);
    try {
      localStorage.removeItem('docsum_history');
    } catch (e) {
  ...
  const handleGenerateSummary = async (customParam?: string) => {
    ...
    const activeLabels = focusPoints.filter((f) => f.isSelected).map((f) => f.label);
